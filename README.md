...
Private web app for importing Australian bank CSV exports, deduplicating rows, reviewing and classifying transactions, flagging likely internal transfers, and calculating auditable profit and balance views.
...
- `/` — dashboard: income, expenses, profit, balance views, bank balance, and exclusion counts
- `/imports` — upload bank CSVs and view per-import summaries
- `/review` — review queue for unresolved transactions and suggested exclusions
- `/transactions` — full transaction archive with filters, inline edits, and audit trail
- `/settings` — configurable import classification rules
...
- Current bank balance is derived from the latest imported balance-bearing transaction

### Classification model
The app supports:
- income
- business expenses
- shared or personal allocation categories
- excluded rows for transfers, refunds, reversals, and similar non-operating movements

Positive transactions may only be classified as income or excluded.
Negative transactions may be classified into expense/allocation categories or excluded.
...
| `REVIEWED` | A classification decision has been saved |
| `CONFIRMED_EXCLUSION` | The exclusion has been confirmed |

### Transfer detection
Transactions are auto-flagged as `SUGGESTED_EXCLUSION` when transfer-like text patterns match across transaction type, details, merchant name, or category, or when a likely cross-account pair is detected within a short date window.
...
### Bulk actions
From the review queue or transaction archive, the operator can:
